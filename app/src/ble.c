...
int update_advertising(void) {
    ...
    if (zmk_ble_active_profile_is_open()) {
        ...
        // macOS/iOS ignore directed advertising (Apple BLE spec restriction), so
        // directed adv only wastes DIR_ADV_TIMEOUT before falling back to open.
        // Advertise open directly so the host reconnects immediately after power-on.
